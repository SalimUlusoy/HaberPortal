...
{
    public partial class UC_Seyahat : UserControl
    {
        public UC_Seyahat()
        {
            InitializeComponent();
        ...
        {
            var habers = haberler.Where(h => h.KategoriNo == 3).ToList();
            panel_SeyahatHaberleri.Controls.Clear();
            int x = 70, y = 10;
            ...
            for (int i = 0; i < habers.Count(); i++)
            {
                try
                {
                    ...
                    panel_SeyahatHaberleri.Controls.Add(haberTemp[i]);
                    /*
                     * 
                     */
