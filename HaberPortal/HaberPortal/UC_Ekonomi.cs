...
{
    public partial class UC_Ekonomi : UserControl
    {
        public UC_Ekonomi()
        {
            InitializeComponent();
        ...
        {
            var habers = haberler.Where(h => h.KategoriNo == 4).ToList();
            panel_EkonomiHaberleri.Controls.Clear();
            int x = 70, y = 10;
            ...
            for (int i = 0; i < habers.Count(); i++)
            {
                try
                {
                    ...
                    panel_EkonomiHaberleri.Controls.Add(haberTemp[i]);
                    /*
                     * 
                     */
