...
{
    public partial class UC_Spor : UserControl
    {
        public UC_Spor()
        {
            InitializeComponent();
        ...
        {
            var habers = haberler.Where(h => h.KategoriNo == 6).ToList();
            panel_SporHaberleri.Controls.Clear();
            int x = 70, y = 10;
            ...
            for (int i = 0; i < habers.Count(); i++)
            {
                try
                {
                    ...
                    panel_SporHaberleri.Controls.Add(haberTemp[i]);
                    /*
                     * 
                     */
